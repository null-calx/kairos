...
#+BEGIN_SRC sh :results file none :file ssl.out.txt
  mkdir -p cert/cc cert/org

  openssl genrsa \
  	-out cert/cc/istaroth.key 2048
  openssl req \
  	-new \
  	-key cert/cc/istaroth.key \
  	-out cert/cc/istaroth.csr \
  	-config cert/istaroth.cc.conf

  openssl genrsa \
  	-out cert/org/istaroth.key 2048
  openssl req \
  	-new \
  	-key cert/org/istaroth.key \
  	-out cert/org/istaroth.csr \
  	-config cert/istaroth.org.conf
#+END_SRC
...
#+BEGIN_SRC sh :results file none :file certbot.out.txt
  ...
  mv 0000_cert.pem cert/cc/cert.pem
  mv 0000_chain.pem cert/cc/chain.pem
  mv 0001_chain.pem cert/cc/fullchain.pem

  certbot certonly \
  	--csr cert/org/istaroth.csr \
  	--preferred-challenges dns \
  	--config-dir certbot \
  	--work-dir certbot \
  	--logs-dir certbot \
  ...
  mv 0000_cert.pem cert/org/cert.pem
  mv 0000_chain.pem cert/org/chain.pem
  mv 0001_chain.pem cert/org/fullchain.pem

  rm cert/cloudflare.ini
#+END_SRC
...
#+BEGIN_SRC sh :results file none :file push-ssl.out.txt :var REMOTE_USER=server[0] :var REMOTE_DOMAIN=server[1]
  ...
  tar cf cert.tar cert/cc cert/org
  gzip cert.tar
